...
import { ROUTER_ADDRESS, FACTORY_ADDRESS, QUOTE_SYMBOL, QUOTE_ADDRESS, QUOTE_DECIMALS, DEX_TOKEN_OPTIONS, QUOTE_TOKEN, TOKENS } from "@/constants/tokens";
...
const TOKEN_OPTIONS = DEX_TOKEN_OPTIONS.map((t) => t.symbol) as readonly string[];
type TokenOutSymbol = typeof DEX_TOKEN_OPTIONS[number]["symbol"];
...
export default function Swap() {
  ...
  // Read pair address from factory (based on tokenIn and tokenOut)
  ...
  // Read pair token0/token1 and reserves
  ...
  const eurcBalanceFmt = useMemo(() => {
    if (!eurcBalance) return "0";
    return formatUnits(eurcBalance.value, QUOTE_DECIMALS);
  }, [eurcBalance]);
  ...
  const [amountOutExpected, minOutFormatted] = useMemo(() => {
    try {
      if (!amountIn || !reserves || !token0 || !token1 || !tokenInConfig || !tokenOutConfigFinal) {
        return ["0", "0"];
      }
      ...
      const token1Addr = (token1 as string).toLowerCase();
      const tokenInAddr = tokenInConfig.address.toLowerCase();
      const tokenOutAddr = tokenOutConfigFinal.address.toLowerCase();
      
      // Set reserves based on tokenIn and tokenOut
      let reserveIn: bigint;
      let reserveOut: bigint;
      
      const reservesArray = reserves as Reserves | null;
      if (token0Addr === tokenInAddr) {
        // tokenIn is token0
        reserveIn = reservesArray?.[0] ?? BigInt(0);
        reserveOut = reservesArray?.[1] ?? BigInt(0);
      } else {
        // tokenIn is token1
        reserveIn = reservesArray?.[1] ?? BigInt(0);
        reserveOut = reservesArray?.[0] ?? BigInt(0);
      }
  ...
  }, [amountIn, reserves, token0, token1, tokenInConfig, tokenOutConfigFinal, slippagePct]);

  // Compute price impact (approx) using before/after price
  const priceImpactPct = useMemo(() => {
    try {
      if (!amountIn || !reserves || !token0 || !token1 || !tokenInConfig) return 0;
      const amtIn = parseUnits(amountIn, tokenInConfig.decimals);
      const isQuoteToken0 = (token0 as string).toLowerCase() === QUOTE_ADDRESS.toLowerCase();
      const isTokenInQuote = tokenInConfig.address.toLowerCase() === QUOTE_ADDRESS.toLowerCase();
  ...
  }, [amountIn, reserves, token0, token1, tokenInConfig]);
  ...
  const exchangeRate = useMemo(() => {
    try {
      if (!reserves || !token0 || !token1 || !tokenInConfig || !tokenOutConfigFinal) return "0";
      const token0Addr = (token0 as string).toLowerCase();
      const tokenInAddr = tokenInConfig.address.toLowerCase();
      ...
      // Rate = reserveOut / reserveIn, adjusted for decimals
      const decimalsDiff = tokenOutConfigFinal.decimals - tokenInConfig.decimals;
      const rate = (Number(reserveOut) / Number(reserveIn)) * Math.pow(10, -decimalsDiff);
      return rate.toFixed(4);
    } catch {
      return "0";
    }
  }, [reserves, token0, token1, tokenInConfig, tokenOutConfigFinal]);
  ...
  const onSwap = async () => {
    if (!isConnected || !routerAddress || !tokenInConfig || !tokenOutConfigFinal) {
      showErrorToast("Please connect wallet");
      return;
    }
    try {
      ...
      const minOut = parseUnits(minOutFormatted || "0", tokenOutConfigFinal.decimals);

      writeContract({
        address: routerAddress,
        abi: ROUTER_ABI,
        functionName: "swapExactTokens",
        args: [tokenInConfig.address, tokenOutConfigFinal.address, amtIn, minOut],
      });
  ...
  }, [isPending, isConfirming, isConfirmed, lastAction, refetchTokenInBalance, refetchTokenOutBalance]);
      ...
      <div className="w-[35%] mx-auto">
        <div className="relative bg-secondary border-custom-2 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4 mt-0">
            ...
            <button
              ...
              className={`p-1 rounded-lg transition-colors ${
                showSettings ? "bg-gray-200" : "hover-bg-custom"
              }`}
              title="Settings"
            >
          ...
          <div className="space-y-4">
            ...
            {showSettings && (
              <div className="rounded-lg border-custom-2 p-4 bg-secondary-2">
                  ...
                  {["0.1", "0.5", "1.0"].map((preset) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border transition-colors ${
                        userEditedSlippage && slippagePct === preset
                        ...
                        {ALL_TOKEN_OPTIONS.filter((t) => t.symbol !== tokenOutSymbol).map((token) => (
                          <button
                            ...
                            className={`w-full px-3 py-2 flex items-center gap-2 hover:bg-gray-700 transition-colors ${
                              tokenInSymbol === token.symbol ? 'bg-gray-700' : ''
                            }`}
                          >
                            <img
                              src={token.icon}
                      ...
                      <div className="absolute top-full left-0 mt-1 w-full bg-secondary border-custom-2 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
                        {ALL_TOKEN_OPTIONS.filter((t) => t.symbol !== tokenInSymbol).map((token) => (
                          <button
                            ...
                            className={`w-full px-3 py-2 flex items-center gap-2 hover:bg-gray-700 transition-colors ${
                              tokenOutSymbol === token.symbol ? 'bg-gray-700' : ''
                            }`}
                          >
                            <img
                              src={token.icon}
            ...
            <div className="rounded-lg border-custom-2 p-3 space-y-3 bg-secondary-2">
              {/* Slippage row */}
